...
public class Solution {
    ...
    public static boolean isPointInPolygon(Point point, List<Point> polygon)
    {
            ...
            {
                count *= check(polygon.get(i), polygon.get(i + 1), point);
            }
          //  count *= check(polygon.get(0), polygon.get(polygon.size() - 1), point);
            count *= check(polygon.get(polygon.size() - 1), polygon.get(0), point);
        if (count<0)
            return true;
        else return false;
        }
