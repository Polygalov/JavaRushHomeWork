...
public class Solution {
    ...
    public <T extends Item> T getProxy(Class<T> clazz, Class<?>... classes){
        Class[] mas = new Class[classes.length + 1];
        System.arraycopy(classes, 0, mas, 0, classes.length);
        mas[mas.length-1] = clazz;
        return (T) Proxy.newProxyInstance(clazz.getClassLoader(), mas, new ItemInvocationHandler());
    }
}
